...
RETURNS TRIGGER AS $$
BEGIN
  -- If a new borrow record is created or updated to 'Đang mượn thẻ' or 'Ra chưa trả thẻ'
  IF (TG_OP = 'INSERT' AND (NEW.trang_thai = 'Đang mượn thẻ' OR NEW.trang_thai = 'Ra chưa trả thẻ')) OR 
     (TG_OP = 'UPDATE' AND (NEW.trang_thai = 'Đang mượn thẻ' OR NEW.trang_thai = 'Ra chưa trả thẻ') AND OLD.trang_thai NOT IN ('Đang mượn thẻ', 'Ra chưa trả thẻ')) THEN
      UPDATE dm_the_cham
  ...
  ELSIF (TG_OP = 'UPDATE' AND NEW.trang_thai IN ('Đã trả thẻ', 'Trả thẻ chưa trả tiền') AND OLD.trang_thai NOT IN ('Đã trả thẻ', 'Trả thẻ chưa trả tiền')) THEN
      ...
      IF NOT EXISTS (
          ...
          WHERE so_the = NEW.so_the 
          AND trang_thai IN ('Đang mượn thẻ', 'Ra chưa trả thẻ')
          AND id <> NEW.id
      ) THEN
          UPDATE dm_the_cham
