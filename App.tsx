...
import { getPersonnel, bulkCreatePersonnel, createPersonnel, updatePersonnel, getEmployeeDetails, Employee, Family, WorkHistory, Training, Salary } from './services/personnelService';
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      ...
      const newEmployees: Omit<Employee, 'id' | 'created_at'>[] = rows.map(row => ({
        ...
        dien_thoai: row[11],
        thang_nam_tuyen_dung: row[12],
        thang_nam_nhap_ngu: row[13],
        ngay_ve_khoa_cong_tac: row[14],
        trang_thai: row[15],
        dien_quan_ly: row[16],
        ngay_vao_dang: row[17],
        ngay_chinh_thuc: row[18],
        so_the_dang: row[19],
        ngay_cap_the_dang: row[20],
      ...
      if (newEmployees.length > 0) {
        try {
          ...
          alert("Có lỗi xảy ra khi nhập dữ liệu. Vui lòng kiểm tra lại file.");
        }
      }
    };
                ...
                {personnel.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-6 py-8 text-center text-slate-500">
                      Chưa có dữ liệu nhân viên. Hãy thêm nhân viên mới.
                    </td>
                  </tr>
                ) : (
                  personnel.map((emp) => (
        ...
        <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
          ...
          <main className="flex-1 overflow-y-auto bg-slate-50 scrollbar-hide">
            <Routes>
              ...
              <Route path="/leave" element={<PlaceholderPage title="Quản lý Phép" />} />
